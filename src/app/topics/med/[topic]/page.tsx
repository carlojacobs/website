// src/app/topics/med/[topic]/page.tsx
...
export default async function MedTopicPage(props: {
  params: Promise<{ topic: string }>;
}) {
  const { topic } = await props.params;

  const allEntries = medSource
    .getPages()
    .filter((p) => !p.data.draft)
    .sort((a, b) => toMillis(b.data.created) - toMillis(a.data.created));

  const matching = allEntries.filter((p) => {
    ...
    <main>
      <JournalHeader
        strip={{
          ...
          right: <span>{matching.length} {matching.length === 1 ? "entry" : "entries"}</span>,
        }}
      />
...
export async function generateStaticParams() {
  const entries = medSource.getPages().filter((p) => !p.data.draft);
